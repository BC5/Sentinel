...
public class UtilityCommands : InteractionModuleBase
{
    ...
    */
    ...
    [Discord.Interactions.RequireOwner]
    [SlashCommand(name: "addautoresponse", description: "Add an autoresponse")]
    public async Task AddResponse(string trigger, string response, bool wildcard = false)
    {
        Config cfg = _core.GetConfig();

        trigger = trigger.ToUpper();
        
        Config.AutoResponse ar = new Config.AutoResponse()
        {
            Trigger = trigger,
            Wildcard = wildcard
        };

        if (Emote.TryParse(response, out var emote))
        {
            ar.EmoteResponse = emote.ToString();
        }
        else if (Emoji.TryParse(response, out var emoji))
        {
            ar.EmoteResponse = emoji.ToString();
        }
        else
        {
