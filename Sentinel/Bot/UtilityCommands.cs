...
public class UtilityCommands : InteractionModuleBase
{
    ...
    public UtilityCommands(SentinelBot sentinel, OCRManager ocr, ProcedureScheduler procSched)
    {
        _bot = sentinel;
        _ocr = ocr;
    ...
    public async Task ExecuteProcedure(string proc, string? schedule = null)
    {
        ...
        proc = proc.ToUpper();
        await DeferAsync();
        string procpath = $@"{_bot.GetConfig().DataDirectory}/procedures/{proc}.xml";
    ...
    {
        await _bot.GetClient().SetGameAsync(message, type: type);
        await RespondAsync("Done", ephemeral: true);
    }
