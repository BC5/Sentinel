...
public class SentinelEvents
{
    ...
    public SentinelEvents(SentinelBot bot)
    {
        _bot = bot;
    }
    ...
    public async Task MessageRemove(Cacheable<IMessage, ulong> msg, Cacheable<IMessageChannel, ulong> channel)
    {
        ...
        if (handler != null)
        {
            SentinelDatabase db = new SentinelDatabase($@"{_bot.GetConfig().DataDirectory}/data.sqlite");
...
{
    public static async Task<MessageEditContext> Create(SocketMessage msg, Cacheable<IMessage, ulong> old, SentinelDatabase data)
    {
    ...
    public SentinelDatabase DatabaseContext { get; set; }
}

public class ReactContext
{
    public static async Task<ReactContext> Create(SocketReaction react, IUserMessage message, SentinelDatabase data, bool remove)
    {
...
{
    public static async Task<MessageContext> Create(SocketMessage msg, SentinelDatabase data)
    {
...
{
    public static async Task<MessageRemoveContext> Create(Cacheable<IMessage, ulong> msg, Cacheable<IMessageChannel, ulong> channel, SentinelDatabase data)
    {
