...
public class Sentinel
{
    ...
    /*
    private async Task AntiSierraAktion(SocketMessage msg)
    {
        if (msg.Embeds.Count == 0)
        {
            Console.WriteLine("No embed");
            return;
        }
        var embed = msg.Embeds.First();
        if(!embed.Author.HasValue) return;
        if (embed.Author.Value.Name.Contains("Mute"))
        {
            ulong muid = MentionUtils.ParseUser(embed.Fields[1].Value);
            if (muid is 718874441567699086 or 661009647280324633)
            {
                Console.WriteLine("Lolcow Mod Mute via Dyno Detected");
                ulong uid = MentionUtils.ParseUser(embed.Fields[0].Value);
                var usr = ((SocketGuildChannel) (msg.Channel)).Guild.GetUser(uid);
                if(usr.Roles.Contains(usr.Guild.GetRole(1019250206212116571)))
                {
                    var mod = ((SocketGuildChannel) (msg.Channel)).Guild.GetUser(muid);
                    await UnmuteDM(usr, mod.Username);
                }
            }
        }
    }
    */
    ...
    /*
    private async Task AntiSierraAktion(Cacheable<SocketGuildUser, ulong> before, SocketGuildUser after)
    {
        try
        {
            if (IsTimedOut(after.TimedOutUntil) && !IsTimedOut(before.Value.TimedOutUntil) &&
                after.Roles.Contains(after.Guild.GetRole(1019250206212116571)))
            {
                bool lolcowmute = false;
                IUser? lolcowmod = null;
                await foreach (var entries in after.Guild.GetAuditLogsAsync(1))
                {
                    var entry = entries.First();
                    if (entry.Action == ActionType.MemberUpdated)
                    {
                        if (entry.User.Id is 718874441567699086 or 661009647280324633)
                        {
                            Console.WriteLine("Lolcow Mod Mute Detected");
                            lolcowmute = true;
                            lolcowmod = entry.User;
                        }
                    }
                }
                ...
                if (lolcowmute)
                {
                    await UnmuteDM(after,lolcowmod.Username);
                }
            }
        }
        catch (Exception e)
    ...
    */
    ...
    /*
    private async Task UnmuteDM(SocketGuildUser after, string muter)
    {
        var dms = await after.CreateDMChannelAsync();
        var eb = new EmbedBuilder();
        eb.WithTitle("Uh Oh!");
        eb.WithDescription(
            $"Looks like {muter} muted you in TCv3!\nFortunately I'm willing to bail you out if it's an unreasonable mute! <:troll:989198639710752799>\n\nIf {muter} is having a {muter} moment, hit the button below to unmute yourself.\nIf you actually did break the rules, don't push it. You'll probably get me into trouble...");
        eb.WithColor(Color.Purple);
        eb.WithFooter("DO NOT TELL ANY OTHER MODERATORS THAT YOU RECEIVED THIS MESSAGE.");
        var cpnt = new ComponentBuilder();
        cpnt.WithButton("Unmute Me", $"sentinel-unmute-{after.Guild.Id}-{after.Id}", ButtonStyle.Success);
        await dms.SendMessageAsync(embed: eb.Build(), components: cpnt.Build());
        await DMBC5($"I just offered to unmute {after.Mention} ({muter} muted them)");
    }
    */
